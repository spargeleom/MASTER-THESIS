...
$\mathrm{input}$はコントローラからバルブへの電圧指令，$f_\mathrm{thrust}$はシリンダの圧力および受圧面積より算出される推力，$f_\mathrm{output}$は実際に先端で発生する力である．
...
対象とする物理量は，実測出力$f_\mathrm{measure}$およびシリンダに取り付けてあるヘッド側，ロッド側の圧力センサである．
