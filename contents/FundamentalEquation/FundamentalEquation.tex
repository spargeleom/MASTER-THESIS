...
このサーボバルブとシリンダについてJelaliら\cite{jelali2012hydraulic}の手法から抜粋し，第一原理に基づいた基礎方程式の記述を行う．
...
\begin{figure}[t]
    \centering
        ...
        \caption{System of Servo Valve and Cylinder}
        \label{fig:valve-cylinder}
\end{figure}

\section{サーボバルブ}
\subsection{サーボバルブにおける変数設定}
サーボバルブは\figname\ref{fig:valve-cylinder}に示すように,ポンプ側に供給ポート（ポートP）および戻りポート（ポートT），シリンダ側に制御ポート（ポートA，ポートB）がある．
サーボバルブにおいて，ポートPはポンプからの油が流れ込む，ポートTはタンクへ油を流し出すというように1方向に油が流れており，
スプール（\figname\ref{fig:valve}）を変位させることによりポートA，およびポートBへ流れる油の向き・流量を制御する．

スプールの中立点からの変位$x_v$はサーボバルブへ与える指令電圧$u_v$に比例するので，コントローラにより制御することができる．
各ポートにおける圧力および流量を\figname\ref{fig:valve}に示すように$p$および$Q$に下付き添字のポート名をつけたもので表す．
%ポンプからの油が流れ込む供給ポート(ポートP)，タンクへ戻る油が流れ出す戻りポート（ポートT），
%シリンダへ油を流す
...
これらをまとめると，$Q_A$は，\eqnname\eqref{eq:orifice_valve}も考慮すると，
\begin{align}
    \notag
    \label{eq:flow_QA}
    Q_A =& Q_{PA}- Q_{AT} \\ \notag
...
サーボバルブへの指令電圧入力$u_v$とスプール変位$x_v$の関係は，周波数応答などから\eqnname\eqref{eq:valve_freq}に示す二次系の運動方程式で近似して表すことがしられている．
\begin{equation}
    \label{eq:valve_freq}
    \frac{1}{\omega_v^2}\ddot{x}_v^* + \frac{2D_v}{\omega_v}\dot{x}_v^* + x_v^* + f_{hs}\mathrm{sign}(\dot{x}_v^*) = K_v u_v^*
\end{equation}
なお，$u_v^*$や$x_v^*$などはそれぞれ入力電圧の最大値$u_{v,\mathrm{max}}$，スプール変位の最大値$x_{v,\mathrm{max}}$で除して正規化されたものであり，以下の\eqnname\eqref{eq:normalize_u}から\eqnname\eqref{eq:normalize_xv}で定義される．
\begin{align}
    \label{eq:normalize_u}
    u_v^* &= \frac{u_v}{u_{v,\mathrm{max}}}\\
    \label{eq:normalize_xv}
    x_v^* = \frac{x_v}{x_{v,\mathrm{max}}}~,~\dot{x}_v^* &= \frac{\dot{x}_v}{x_{v,\mathrm{max}}}~,~\ddot{x}_v^* = \frac{\ddot{x}_v}{x_{v,\mathrm{max}}}
\end{align}
また，\eqnname\eqref{eq:valve_freq}において$K_v$はバルブのゲイン，$\omega_v$は固有角振動数，$D_v$は粘性係数，$f_{hs}$はバルブのヒステリシスや応答感度を表す関数である．

\section{油圧シリンダ}
油圧シリンダ内の作動流体についてモデル化する．
...
\end{align}
シリンダロッドの速度を$\dot{x}_\mathrm{p}$とし，ロッド（rod）側の受圧面積を$\Ar$,
ヘッド（head）側の受圧面積を$\Ah$とすると，$\dot{V}_\mathrm{rod}$および$\dot{V}_\mathrm{rod}$は
\begin{align}
    \label{eq:dotVrod}
...
\end{align}
と表される．
また，バルブとシリンダの間およびホースにおける圧力降下が無視できるときには，$p_A = p_\mathrm{head}$，$p_B = p_\mathrm{rod}$であり，
...
\end{align}
と表せる．
$F_\mathrm{ext}$はロッドをシリンダ内に押し戻そうとする外力であり，$F_f(\dot{x}_\mathrm{p})$は摩擦である．
...
また，シリンダロッド先端を固定している際には$\dot{x}_\mathrm{p}=\ddot{x}_\mathrm{p}=0$であるため，\eqnname\eqref{eq:valve_freq}および\eqnname\eqref{eq:dp_load}より，バルブへの入力から負荷圧力までの伝達関数は
\begin{align}
    \label{eq:tf_Gpu}
